...
(function () {
    ...
    function pollCard(p) {
        ...
        return `<div class="${ui.CARD} p-4 md:p-5 flex flex-col">
            ...
            <div class="mt-auto pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                ${hasVoted
    ...
    VQ.boot({
        ...
        render() {
            VQ.content(
                ...
                `<div class="space-y-4">${D.polls.map(pollCard).join('')}</div>` +
                ui.groupTitle(t('svIndepth'), 'fa-clipboard-list') +
                D.indepthSurveys.map(sv => `<div class="${ui.CARD} p-4 md:p-5 flex flex-col md:flex-row md:items-center gap-4">
                    <span class="w-14 h-14 rounded-2xl bg-gradient-to-br from-vq-teal to-vq-teal-dark text-white text-xl flex items-center justify-center shrink-0"><i class="fa-solid fa-clipboard-list"></i></span>
