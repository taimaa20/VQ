...
(function () {
    ...
    function browser() {
        ...
        return `<div class="${ui.CARD} overflow-hidden">
            <div class="p-4 md:p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-3">
                <span class="w-12 h-12 rounded-xl bg-vq-teal text-white flex items-center justify-center text-lg shrink-0"><i class="fa-solid ${d.icon}"></i></span>
                <div class="flex-1 min-w-0">
                    <h2 class="text-base font-medium text-slate-800">${esc(tx(d.name))}</h2>
                    <p class="text-xs text-slate-500">${esc(tx(d.description))}</p>
                </div>
            </div>
            <div class="p-4">
                <div class="mb-4">${crumbs}</div>
                ${body}
            </div>
        </div>`;
    }
