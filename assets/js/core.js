...
(function () {
    ...
    /* Discussion Board and VQ Structure pages still exist but are not in the menu */

    /* Galleries live in the header, as in the original portal */
    const NAV_HEADER = [
    ...
    function headerHTML() {
        ...
        const galleries = labelCls => NAV_HEADER.map(n => `<a href="${href(n.page)}" class="header-link ${isCurrent(n) ? 'is-active' : ''}" ${isCurrent(n) ? 'aria-current="page"' : ''} title="${t(n.key)}">
            <i class="fa-solid ${n.icon}"></i><span class="${labelCls}">${t(n.key)}</span></a>`).join('');

        return `
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between gap-3 min-h-16 py-2">
                <div class="shrink-0 flex items-center gap-2 lg:gap-4">
                    ...
                    <nav class="hidden sm:flex items-center gap-1" aria-label="${t('navGalleries')}">${galleries('hidden lg:inline')}</nav>
                </div>
            ...
            <nav class="sm:hidden flex items-center gap-1 pb-2" aria-label="${t('navGalleries')}">${galleries('')}</nav>
        </div>`;
    }
    ...
    function navHTML() {
        ...
        const current = NAV_MAIN.concat(NAV_HEADER).find(isCurrent);
    ...
    /* ---------- Side widgets (same blocks and markup as the homepage left rail: 6.6–6.10, 6.12) ---------- */
    ...
    function widgetsHTML() {
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
            ...
            <div class="grid grid-cols-2 gap-2 text-xs max-h-56 overflow-y-auto pe-1">
                ${systems.map(([url, icon, key]) => `<a href="${url}" target="_blank" rel="noopener noreferrer" class="flex flex-col items-center p-2.5 rounded-xl bg-slate-50 hover:bg-vq-teal/5 border border-slate-100 transition">
                    <i class="${icon} text-vq-teal text-base mb-1"></i><span class="text-[11px] text-center">${t(key)}</span></a>`).join('')}
            </div>
        </div>

        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-3 bg-slate-50 border-b border-slate-100">
                <h4 class="text-xs font-medium text-slate-800 flex items-center gap-1.5"><i class="fa-solid fa-bullseye text-vq-teal"></i> <span>${t('hpVision')}</span></h4>
            </div>
            <div class="p-3 space-y-3">
                ${statements.map(([id, color, alt, title, text]) => `<article>
                    <img src="https://images.unsplash.com/${id}?auto=format&fit=crop&w=400&q=80" alt="${alt}" class="w-full h-20 object-cover rounded-xl mb-2"
                        onerror="this.src='https://placehold.co/400x160/${color}/FFFFFF?text=${alt}';">
                    <h5 class="text-xs font-medium text-vq-teal">${t(title)}</h5>
                    <p class="text-[11px] text-slate-600 mt-0.5 leading-relaxed">${t(text)}</p>
                </article>`).join('')}
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-3 bg-slate-50 border-b border-slate-100">
                <h4 class="text-xs font-medium text-slate-800 flex items-center gap-1.5"><i class="fa-solid fa-sitemap text-vq-teal"></i> <span>${t('hpStructure')}</span></h4>
            </div>
            <div class="p-3">
                <a href="${href('structure')}" class="block group">
                    <img src="https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&w=900&q=80" alt="VQ Structure"
                        class="w-full h-28 object-cover rounded-xl group-hover:opacity-90 transition"
                        onerror="this.src='https://placehold.co/800x300/00626C/FFFFFF?text=VQ+Structure';">
                </a>
                <p class="text-[11px] text-slate-600 mt-2 leading-relaxed">${t('hpStructureText')}</p>
            </div>
        </div>
        ...
        document.body.insertAdjacentHTML('afterbegin', `
            ...
            <main class="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <section id="pageContent" class="lg:col-span-6 min-w-0 bg-white rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6 space-y-5"></section>
