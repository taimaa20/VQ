...
(function () {
    ...
    function openGuide(id) {
        ...
        if (g.kind === 'video') {
            ...
            /* One continuous procedure: description, then numbered steps separated by dividers (no card per step) */
            body = `<div class="p-4 sm:p-5">
                <p class="text-xs text-slate-600 leading-relaxed pb-4">${esc(tx(g.description))}</p>
                <ol class="border-t border-slate-100 divide-y divide-slate-100">${g.steps.map((step, i) => `<li class="py-4 grid grid-cols-1 sm:grid-cols-5 gap-4 items-center">
                    <div class="sm:col-span-2 relative rounded-xl overflow-hidden h-28 bg-slate-200">
                    ...
                    <div class="sm:col-span-3 flex items-start gap-3">
                        <span class="w-7 h-7 rounded-full bg-vq-teal text-white text-xs font-medium flex items-center justify-center shrink-0">${i + 1}</span>
                        <div>
                            <p class="text-[10px] text-vq-teal font-medium">${t('ugStep', { n: i + 1 })}</p>
                            <p class="text-sm text-slate-800 mt-0.5 leading-relaxed">${esc(tx(step))}</p>
                        </div>
                    </div>
                </li>`).join('')}</ol>
            </div>`;
        ...
        if (g.kind === 'video') body += `<div class="px-4 pb-4"><p class="text-xs text-slate-600 leading-relaxed">${esc(tx(g.description))}</p></div>`;
