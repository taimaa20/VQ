    ...
    VQ.boot({
        ...
        render() {
            ...
            const article = `<article class="space-y-5">
                <img src="${VQ.photo(ty.image)}" alt="" class="w-full aspect-[16/8] object-cover rounded-2xl">
                <div class="space-y-3 text-sm text-slate-700 leading-loose">${ui.paragraphs(a.body)}</div>
                <div>${ui.facts([
                ...
                ${a.attachment ? `<div class="flex flex-wrap items-center gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50">
                ...
                ${a.link ? `<div><a href="${VQ.href(a.link.page)}" class="${ui.BTN.primary}">${esc(tx(a.link.label))} ${ui.arrow()}</a></div>` : ''}
